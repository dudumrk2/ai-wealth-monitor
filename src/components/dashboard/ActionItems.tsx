...
export default function ActionItems() {
  const [items, setItems] = useState(MOCK_DATA.action_items);
      ...
      <div className="p-5 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
         <h2 className="font-bold text-lg text-slate-800 flex items-center gap-2">
            ...
            <span className="bg-blue-100 text-blue-700 text-xs py-0.5 px-2.5 rounded-full font-bold">
              {items.filter(i => !i.is_completed).length} Requires Attention
            </span>
         </h2>
      </div>
                  ...
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1">
                      <div className="flex gap-2 items-center mb-1">
                        {!item.is_completed && <Icon className={clsx("w-4 h-4", color)} />}
