  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm mb-6">
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <div>
          <h2 className="font-bold text-slate-800 text-sm leading-tight">העלאת דוחות פנסיה</h2>
          <p className="text-slate-400 text-xs">עד {MAX_FILES} קבצי PDF — מעובדים ישירות בענן בלי שמירה מקומית</p>
        </div>
      </div>
      ...
      >
        ...
        <div className={clsx(
          ...
          <Upload className={clsx('w-5 h-5', isDragOver ? 'text-blue-600' : 'text-slate-400')} />
        </div>
        <p className="text-sm font-medium text-slate-600">
          {isDragOver ? 'שחרר כאן…' : 'גרור ושחרר קבצי PDF, או לחץ לבחירה'}
        </p>
        <p className="text-xs text-slate-400">PDF בלבד · עד {MAX_FILES} קבצים</p>
      </div>
      ...
      {selectedFiles.length > 0 && (
        <div className="mt-3 space-y-2">
          {selectedFiles.map((file, i) => (
            <div
              key={`${file.name}-${i}`}
              className="flex items-center justify-between px-3 py-2 bg-blue-50 rounded-lg border border-blue-100"
            >
              ...
              <button
                ...
                className="p-1 rounded-md hover:bg-blue-100 text-slate-400 hover:text-slate-600 transition-colors flex-shrink-0"
      ...
      {statusMessage && (
        <div className={clsx(
          ...
          uploadState === 'error'   && 'bg-red-50 text-red-700 border border-red-100',
          uploadState === 'success' && 'bg-green-50 text-green-700 border border-green-100',
          uploadState === 'uploading' && 'bg-blue-50 text-blue-700 border border-blue-100',
        )}>
