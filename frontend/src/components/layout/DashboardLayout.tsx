...
export default function DashboardLayout({ children, onRefresh, isRefreshing }: DashboardLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col font-sans text-slate-900 dark:text-slate-100 transition-colors duration-200">
      ...
      <header className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 h-16 flex items-center justify-between px-4 md:px-8 sticky top-0 z-50 shadow-sm">

        {/* Logo */}
        <div className="flex items-center gap-3 font-bold text-lg text-slate-800 dark:text-slate-100 tracking-tight">
        ...
        <div className="flex items-center gap-2">
          ...
          {onRefresh && (
            <button
              ...
              className="p-2 text-slate-500 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-slate-800 rounded-lg transition-all active:scale-90 disabled:opacity-50 disabled:cursor-not-allowed"
              title="רענן"
            >
          ...
          <button
            ...
            className="flex items-center gap-2 text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-100 hover:bg-slate-100 dark:hover:bg-slate-800 px-3 py-2 rounded-lg transition-colors text-sm font-medium"
            title="הגדרות"
          >
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div className="hidden md:block leading-tight">
              <p className="text-sm font-semibold text-slate-800 dark:text-slate-100 leading-none">{user?.displayName || 'משתמש'}</p>
              <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5" dir="ltr">{user?.email}</p>
            </div>
          </div>
