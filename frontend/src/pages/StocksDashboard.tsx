...
const SortableTh: React.FC<SortableThProps> = ({ label, sortKey, current, dir, onSort, className }) => {
  const isActive = current === sortKey;
  return (
    <th
      onClick={() => onSort(sortKey)}
      className={clsx("px-4 py-3 text-right text-xs font-bold text-slate-400 uppercase tracking-wider whitespace-nowrap cursor-pointer select-none hover:text-slate-600 dark:hover:text-slate-200 transition-colors group", className)}
    >
