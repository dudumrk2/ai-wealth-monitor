  ...
  "products": [
    {
      ...
      "provider_name": "string (שם חברת הביטוח/בית ההשקעות - כגון הפניקס, אלטשולר, הראל. אזהרה: לעולם אל תכניס את שם המעסיק כמו אמדוקס או מנפאואר)",
...
- תיק מנוהל (Managed Portfolio) — גם אם מופיע בשורה עם שם ספק פנסיוני
- מוצרי "חיסכון פיננסי" של חברות כמו כלל חיסכון פיננסי, מגדל שוקי הון וכדומה
מוצרים אלו אינם מוצרים פנסיוניים ואין להכניסם ל-JSON.

כלל מספר פוליסה חשוד: אם מספר הפוליסה הוא placeholder בוטה (כמו 11111, 00000, 12345) — אל תכלול את המוצר הזה בתוצאה. מספר פוליסה חוקי בישראל הוא בדרך כלל 7-10 ספרות ייחודיות.
...
כלל זיהוי שם ספק (provider_name):
- חובה לחלץ את שם הגוף הפיננסי המנהל את הכספים (כמו: הפניקס, הראל, מגדל, אלטשולר שחם, כלל, מנורה וכו').
- בשום אופן אין לחלץ את שם המעסיק (למשל: אמדוקס, מנפאואר) תחת provider_name, גם אם הוא מופיע בטבלה תחת עמודת "מעסיק" או בכל מקום אחר בדו"ח. תמיד חפש בעמודת ה"מוצר" או בכותרת הדו"ח מי מנהל את הכסף בפועל.
